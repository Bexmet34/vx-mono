...
/**
 * Albion Avrupa sunucusunda lonca arar
 */
async function searchGuild(query) {
    return new Promise((resolve, reject) => {
        ...
        https.get(url, { family: 4 }, (res) => {
            ...
            res.on('end', () => {
                try {
                    const parsed = JSON.parse(data);
                    // Sadece loncaları (guilds) döndür
                    resolve(parsed.guilds || []);
                } catch (e) {
                    reject(new Error('Lonca araması sırasında hata oluştu.'));
                }
            });
        }).on('error', (err) => reject(err));
    });
...
/**
 * Lonca için son kill verilerini çeker
 */
async function getGuildKills(guildId) {
    return new Promise((resolve, reject) => {
        // En son 50 kill'i çek (Filtreleme servis içinde yapılacak)
        const url = `https://gameinfo-ams.albiononline.com/api/gameinfo/guilds/${guildId}/top?range=day`;
        https.get(url, { family: 4 }, (res) => {
            let data = '';
            res.on('data', (chunk) => data += chunk);
            res.on('end', () => {
                try {
                    ...
                    reject(new Error('Kill verileri çekilemedi.'));
                }
            });
        }).on('error', (err) => reject(err));
    });
