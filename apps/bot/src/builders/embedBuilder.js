    ...
    for (const field of rollerFields) {
        ...
        for (const entry of entries) {
            ...
            if (lineMatch) {
                ...
                let userId = lineMatch[2] || null;

                // Fallback for cases where the regex stops at the colon
                if (!userId && firstLine.includes('<@')) {
...
function buildRolesFields(rolesWithMembers, lang = 'tr', guildOrClient = null) {
    ...
    rolesWithMembers.forEach((item, index) => {
        ...
        if (isHeader) {
            ...
            let line = `${emoji} **${displayRole}**: ${mention}`;
            // Eşyaları (gear) ana embedde göstermemek için aşağıdaki bloğu kaldırdık
            // if (gearInfo) {
            //     line += `\n${gearInfo}`;
            // }
 ...
 */
function createObjectiveInfoEmbed(lang = 'tr') {
    const isTr = lang === 'tr';
    const title = isTr ? '🐍 Objektif Takip Sistemi' : '🐍 Objective Tracking System';
    const description = isTr 
        ? 'Bu kanal üzerinden Albion Online içerisindeki objektifleri (Sandık, Core vb.) bildirebilir ve takip edebilirsiniz.\n\n' +
          '**Nasıl Çalışır?**\n' +
          '1. Aşağıdaki **"Objektif Oluştur"** butonuna basın.\n' +
          '2. Açılan formda Harita, Etkinlik ve Kalan Süreyi girin.\n' +
          '3. Bildirim kanalında paylaşılan objektife **"Geliyorum"** diyerek katılın.'
        : 'You can report and track Albion Online objectives (Chests, Cores, etc.) through this channel.\n\n' +
          '**How it Works?**\n' +
          '1. Click the **"Create Objective"** button below.\n' +
          '2. Enter the Map, Event, and Remaining Time in the form.\n' +
          '3. Join the objective by clicking **"I\'m coming"** in the notification channel.';
...
/**
 * Creates a premium player info card for registration
 */
function createPlayerCardEmbed(player, lang = 'tr') {
    const { formatFame } = require('../services/albionService');
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { 
                name: lang === 'tr' ? '⚔️ PvP Bilgileri' : '⚔️ PvP Stats', 
                value: `**Kill Fame:** \`${formatFame(player.KillFame)}\`\n**Death Fame:** \`${formatFame(player.DeathFame)}\`\n**K/D Ratio:** \`${(player.KillFame / (player.DeathFame || 1)).toFixed(2)}\``, 
                inline: true 
            },
            { 
                name: lang === 'tr' ? '🛡️ PvE Bilgileri' : '🛡️ PvE Stats', 
                value: `**Total PvE:** \`${formatFame(player.LifetimeStatistics?.PvE?.Total || 0)}\`\n**Royal:** \`${formatFame(player.LifetimeStatistics?.PvE?.Royal || 0)}\`\n**Outlands:** \`${formatFame(player.LifetimeStatistics?.PvE?.Outlands || 0)}\``, 
                inline: true 
            },
            { 
            ...
            { 
                name: lang === 'tr' ? '⚒️ Meslekler' : '⚒️ Gathering', 
                value: `**Gathering:** \`${formatFame(player.LifetimeStatistics?.Gathering?.All?.Total || 0)}\`\n**Crafting:** \`${formatFame(player.LifetimeStatistics?.Crafting?.Total || 0)}\``, 
                inline: true 
            },
            { 
                name: lang === 'tr' ? '📈 Diğer' : '📈 Others', 
                value: `**Guild:** \`${player.GuildName || '-'}\`\n**Alliance:** \`${player.AllianceName || '-'}\``, 
                inline: true 
            },
            { 
                name: '\u200b', value: '\u200b', inline: true // Alignment spacer
        ...
        .setFooter({ text: `Veyronix Registration System • ID: ${player.Id}` })
        .setTimestamp();
...
module.exports = {
    ...
    createDonateEmbed,
    parseEmbedData,
    createObjectiveEmbed,
    createObjectiveInfoEmbed,
    createPlayerCardEmbed
};
