...
function createSelectMenuPartyComponents(rolesList, ownerId, lang, rolesWithMembers, guildOrClient = null) {
    ...
    // --- Row 3: Voice Channel Link Button & Save Template ---
    rows.push(new ActionRowBuilder().addComponents(
        new ButtonBuilder()
        ...
        new ButtonBuilder()
            .setCustomId('save_temp_init')
            .setLabel(lang === 'tr' ? '💾 Şablonu Kaydet' : '💾 Save Template')
            .setStyle(ButtonStyle.Secondary)
    ));
...
function createButtonPartyComponents(rolesList, ownerId, lang, guildOrClient = null) {
    ...
    // Voice Channel Link Button & Save Template (add if we have space, we should since max is 5, role buttons take max 3, leave+manage is 1 or 2)
...
function createObjectiveButtons(objectiveId, count = 0, lang = 'tr') {
    ...
    return [
        new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                .setCustomId(`obj_join_${objectiveId}`)
                .setLabel(label)
                .setStyle(ButtonStyle.Primary)
                .setEmoji('💪')
        )
...
/**
 * Creates the setup button for the static message
 */
function createObjectiveSetupButtons(lang = 'tr') {
    return [
        new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                .setCustomId('obj_open_modal')
                .setLabel(lang === 'tr' ? 'Objektif Oluştur' : 'Create Objective')
                .setStyle(ButtonStyle.Success)
                .setEmoji('➕')
        )
    ];
...
module.exports = {
    ...
    createClosedObjectiveButtons,
    createObjectiveSetupButtons
};
