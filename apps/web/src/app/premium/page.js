...
export default function PremiumPage() {
  ...
  return (
    <>
      <Navbar />
      <main className="pt-32 min-h-screen pb-20 px-4 md:px-0">
        <section className="relative max-w-3xl mx-auto text-center flex flex-col items-center">
          ...
          <h1 className="font-headline-xl text-4xl md:text-5xl text-on-surface mb-4 font-bold tracking-tight z-10">
            {t.premiumTitle}
          </h1>
          
          <p className="font-body-md text-on-surface-variant max-w-lg mx-auto mb-12 z-10">
            {t.premiumSubtitle}
          </p>
            ...
            <div className={`glass-panel border rounded-lg transition-all duration-300 overflow-hidden ${openAccordion === 'vote' ? 'border-[#5865F2] shadow-[0_0_20px_rgba(88,101,242,0.15)] bg-surface-container-high' : 'border-outline-variant bg-surface-container-low hover:border-outline-variant/80 hover:bg-surface-container-high'}`}>
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-md bg-[#5865F2]/10 border border-[#5865F2]/30 flex items-center justify-center transition-transform group-hover:scale-105">
                    <Settings className="text-[#5865F2]" size={20} />
                  </div>
                  <div className="text-left">
                    <h3 className="font-headline-md text-lg text-on-surface font-bold">{t.premiumVoteTitle}</h3>
              ...
              <div className={`px-6 transition-all duration-300 ease-in-out ${openAccordion === 'vote' ? 'max-h-[500px] pb-6 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                <div className="pt-4 border-t border-outline-variant/50">
                  <p className="text-sm text-on-surface-variant mb-4">{t.premiumVoteDesc1}</p>
                  <p className="text-sm text-on-surface-variant mb-6">{t.premiumVoteDesc2}</p>
                  
                  <h4 className="font-bold text-on-surface mb-2">{t.premiumVoteHowTo}</h4>
                  <p className="text-sm text-on-surface-variant mb-4">{t.premiumVoteHowToDesc}</p>
                  
                  <div className="flex flex-wrap gap-4">
                    <a href="https://top.gg/bot/1082239904169336902/vote" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-3 bg-[#5865F2] hover:bg-[#4752C4] text-white rounded-md font-bold text-sm transition-all shadow-lg hover:shadow-xl active:scale-95">
      ...
      {showCheckout && selectedPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-4 overflow-y-auto">
          ...
          <div className="relative z-10 w-full max-w-4xl my-auto py-6">
            <div className="glass-panel glow-gold flex flex-col p-4 md:p-8 relative border border-primary-container/20">
              ...
              {finalSuccess ? (
                ...
                <div className="animate-slide-up w-full">
                  <div className="text-center mb-6">
                    <h3 className="font-headline-xl text-xl md:text-2xl text-on-surface uppercase tracking-tight mb-2">Güvenli Havale / EFT Akışı</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div className="bg-surface-container p-4 rounded-xl border border-outline-variant text-left">
                    ...
                    <div className="space-y-4 text-left">
                      <div className="bg-[#0B0F19] p-4 border border-primary-container/40 rounded-xl shadow-lg">
                         <div className="font-label-bold text-xs text-primary-container uppercase mb-2">Açıklama Kodu (Zorunlu)</div>
                         <div className="flex items-center justify-between gap-2 mb-2">
                           <div className="font-mono text-2xl md:text-2xl font-bold text-on-surface tracking-widest break-all">{generatedCode}</div>
                           <button onClick={() => navigator.clipboard.writeText(generatedCode)} className="px-3 py-2 bg-primary-container/10 hover:bg-primary-container text-primary-container hover:text-on-primary border border-primary-container/30 rounded-lg shrink-0 text-xs font-bold uppercase transition-colors">
                              Kopyala
                           </button>
                         </div>
                      ...
                      <div>
                         ...
                         <input type="text" placeholder="Örn: Ahmet Yılmaz" value={senderName} onChange={(e) => setSenderName(e.target.value)} className="w-full bg-[#0B0F19] border border-outline-variant p-3 text-on-surface text-sm focus:border-primary-container outline-none rounded-lg" />
                      </div>
                      ...
                      <div className="flex gap-3">
                        <button onClick={() => setShowCheckout(false)} className="flex-1 py-3 border border-outline-variant text-on-surface-variant hover:text-on-surface rounded-lg text-xs uppercase font-bold transition-colors">İptal</button>
                        <button onClick={handleConfirmManualPayment} disabled={isProcessing || senderName.trim().length < 3 || !termsAccepted} className="flex-[2] py-3 bg-primary-container text-on-primary rounded-lg text-xs uppercase font-bold disabled:opacity-40 transition-colors">
                  ...
                  <div className={selectedPlan?.plan_type !== 'user' ? "grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch" : "space-y-4"}>
                          ...
                          userServers.map(s => (
                            <label key={s.guild_id} className={`server-row flex items-center justify-between p-3 border cursor-pointer transition-all ${selectedServer === s.guild_id ? 'border-primary-container/50 bg-primary-container/10' : 'border-outline-variant hover:border-outline'}`}>
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-surface-container-high border border-outline flex items-center justify-center overflow-hidden rounded-md">
                                  {s.icon ? (
                    ...
                    <div className="flex flex-col justify-between h-full space-y-4">
                      ...
                      <button 
                        ...
                        className={`flex-1 py-3 font-label-bold text-sm uppercase transition-all rounded-md ${paymentMethod === 'crypto' ? 'bg-primary-container text-on-primary shadow-[0_0_15px_rgba(255,215,0,0.2)]' : 'bg-surface border border-outline text-on-surface hover:border-primary-container/50'}`}
                      >
                        Kripto İle Öde
                      </button>
                      <button 
                        onClick={() => setPaymentMethod('havale')} 
                        className={`flex-1 py-3 font-label-bold text-sm uppercase transition-all rounded-md ${paymentMethod === 'havale' ? 'bg-primary-container text-on-primary shadow-[0_0_15px_rgba(255,215,0,0.2)]' : 'bg-surface border border-outline text-on-surface hover:border-primary-container/50'}`}
                      >
                        Havale / EFT
                      </button>
                    </div>
                    ...
                    <div className="space-y-4">
                      {status !== "authenticated" ? (
                        <button 
                          ...
                          className="w-full py-5 px-4 bg-[#5865F2] hover:bg-[#4752C4] text-white font-label-bold text-body-md flex items-center justify-center gap-3 transition-all duration-300 shadow-[0_10px_20px_rgba(88,101,242,0.2)] uppercase text-center rounded-md"
                        >
                        ...
                        <div className="space-y-4 bg-surface p-6 border border-outline-variant rounded-md">
                           {bankAccounts.length === 0 ? (
                             ...
                             <>
                               ...
                               <div className="space-y-2 mb-4">
                                  ...
                                  <select 
                                    className="w-full bg-[#0B0F19] border border-outline-variant rounded-md p-4 text-on-surface font-body-md focus:border-primary-container outline-none transition-colors"
