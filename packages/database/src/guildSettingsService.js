...
/**
 * Update general guild settings in supabase
 */
async function updateSupabaseGuildSettings(guildId, settings) {
    try {
        const { error } = await supabase
            .from('guild_settings')
            .upsert({ 
        ...
        console.error(`[Supabase] Error updating settings for ${guildId}:`, error);
        return false;
    }
}
